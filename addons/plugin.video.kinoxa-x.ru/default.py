...
#    else:
    ...
    http = get_url(n_url, cookie = cook, save_cookie = True)
#    if cook=='':
#        cook = re.search('<cookie>(.+?)</cookie>', http).group(1)
    ...
    if i:
        ...
        uri = sys.argv[0] + '?page=' + str(int(page) + 5) + '&url=' + urllib.quote_plus(url)+ '&cook=' + urllib.quote_plus(cook)
        xbmcplugin.addDirectoryItem(pluginhandle, uri, item, True)
        dbg_log('- uri:'+  uri + '\n')        
...
def KNX_view(url, img, name, cook):     
    dbg_log('-NKN_view:'+ '\n')
    dbg_log('- url:'+  url + '\n')
    dbg_log('- img:'+  img + '\n')
    dbg_log('- name:'+  name + '\n')
...
if mode == '': KNX_list(url, page, cook)
